...
/*	if((gAirContionData.SetTemper>30)||(gAirContionData.SetTemper<16))
                                           gAirContionData.SetTemper    = 26;  */    //设置温度
...
void AirCondition_ReadSaveData(void)
{
    ...
    if(gDeveceRelate.UnitNum > 0xF0)    gDeveceRelate.RelateType    = 1 ; //单元号1
	...
	if(gAirContionData.SetMaxTemp > MAX_TEMP)			gAirContionData.SetMaxTemp  = 32;
	if((gAirContionData.SetMinTemp < MIN_TEMP) || (gAirContionData.SetMinTemp > gAirContionData.SetMaxTemp))	gAirContionData.SetMinTemp  = 16;
}
